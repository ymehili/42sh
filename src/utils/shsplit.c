...
void shsplit2(shsplit_t *shsplit)
{
    ...
    if (shsplit->quote_char == '\0' && shsplit->current_token_size > 0)
        shsplit->num_tokens++;
    shsplit->tokens = malloc(sizeof(char *) * (shsplit->num_tokens + 1));
    for (int i = 0; i < shsplit->num_tokens; i++) {
        shsplit->tokens[i] = malloc(sizeof(char) *
            (shsplit->current_token_size + 1));
    }
}

char **shsplit(const char *str)
{
    shsplit_t *shsplit = malloc(sizeof(shsplit_t));
