...
int cd_params_checker(infos_t *infos, char **params, char *last_pwd)
{
    if (get_nb_params(params) != 1 && get_nb_params(params) != 2)
        return return_error(infos, params[0], ": Too many arguments.\n", 1);
    if (get_nb_params(params) != 1 && my_strncmp(params[1], "-", 1) == 0 &&
        last_pwd == NULL)
        return return_error(infos, "", ": No such file or directory.\n", 1);
    return 0;
}
...
int cd_error(infos_t *infos, char *str)
{
    switch (errno) {
        case ENOENT:
            return return_error(infos, str,
                    ": No such file or directory.\n", 1);
        case ENOTDIR:
            return return_error(infos, str, ": Not a directory.\n", 1);
        case EACCES:
            return return_error(infos, str, ": Permission denied.\n", 1);
    }
    return 1;
}
...
int setenv_error_checker(infos_t *infos, char **params)
{
    ...
    if (my_char_isalpha(params[1][0]) == 0)
        return return_error(infos, params[0],
